    ...
    return (
        <Screen>
            <Container>
                <BackgroundImage />
                <Content>
                    <Title text={t("harvest-predicto")} />
                    <Text light={true}>{t("harvest-predicto-desc")}</Text>
                    <Harvest />
                </Content>
...
const WithdrawInfo = ({ state }: { state: FarmingState }) => {
    ...
    const predicto = total && amount.lte(total) ? state.selectedLPToken!.pendingPredicto?.mul(amount).div(total) : null;
    ...
    return (
        <InfoBox>
            <AmountMeta
                amount={predicto ? formatBalance(predicto) : ""}
                suffix={"PREDICT"}
