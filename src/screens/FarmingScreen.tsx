    ...
    return (
        <Selectable
            ...
            <FlexView style={{ alignItems: "center" }}>
                <TokenLogo token={props.token.tokenA} small={true} replaceWBNB={true} />
                <TokenLogo token={props.token.tokenB} small={true} replaceWBNB={true} style={{ marginLeft: 4 }} />
...
const DepositInfo = ({ state }: { state: FarmingState }) => {
    const t = useTranslation();
    const disabled = isEmptyValue(state.amount) || !state.selectedLPToken?.predictoRewardedPerYear;
    const predictoPerYear = disabled
        ? 0
        : parseBalance(state.amount)
              .mul(state.selectedLPToken!.predictoRewardedPerYear!)
              .div(pow10(18));
    return (
        <InfoBox>
            <AmountMeta
                amount={formatBalance(predictoPerYear, 18, 8)}
                suffix={t("predicto-per-year")}
                disabled={disabled}
            />
            <Meta
                label={t("my-balance")}
