...
import { isBNB, parseBalance } from "../utils";
...
const useTokenPairState: () => TokenPairState = () => {
    ...
    const { wrapBNB, unwrapBNB } = useWBNB();
    ...
    const onWrap = useCallback(async () => {
        if (fromAmount && signer) {
            setWrapping(true);
            try {
                const tx = await wrapBNB(parseBalance(fromAmount), signer);
                if (tx) {
                    await tx.wait();
                    await updateTokens();
                    setFromSymbol("");
    ...
    const onUnwrap = useCallback(async () => {
        if (fromAmount && signer) {
            setUnwrapping(true);
            try {
                const tx = await unwrapBNB(parseBalance(fromAmount), signer);
                if (tx) {
                    await tx.wait();
                    await updateTokens();
                    setFromSymbol("");
