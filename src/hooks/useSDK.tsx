...
import { Currency, CurrencyAmount, Fetcher, Pair, TokenAmount, Trade } from "@pancakeswap-libs/sdk-v2";
import { ethers } from "ethers";
import { EthersContext } from "../context/EthersContext";
import Token from "../types/Token";
import { convertToken, isBNB } from "../utils";
...
const useSDK = () => {
    ...
    const getTrade = useCallback(
        async (
            ...
            if (provider) {
                const eth = isBNB(fromToken);
                const from = convertToken(fromToken);
                const to = isBNB(toToken) ? Currency.ETHER : convertToken(toToken);
